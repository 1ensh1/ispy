            ...
            <div style="display:flex;align-items:center;gap:1.25rem;flex-wrap:wrap;">
                <div class="w-20 h-20 rounded-full overflow-hidden shrink-0 bg-indigo-100 flex items-center justify-center">
                    @if($admin->profile_picture)
                        <img src="{{ $admin->profile_picture }}" alt="Profile" style="width:100%;height:100%;object-fit:cover;">
                    @else
                        <span class="text-indigo-700 text-2xl font-bold select-none">{{ strtoupper(substr($admin->name, 0, 1)) }}</span>
                    @endif
                </div>
                <form method="POST" action="{{ route('admin.profile.upload-picture') }}" enctype="multipart/form-data">
                    @csrf
                    <label for="admin-pic-input"
                    ...
                    <input id="admin-pic-input" type="file" name="profile_picture" accept=".jpg,.jpeg,.png,.webp"
